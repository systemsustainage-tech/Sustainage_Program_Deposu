...
def upload_recursive(ftp, local_path, remote_base):
    for root, dirs, files in os.walk(local_path):
        ...
        for file in files:
            # Exclude heavy files to speed up
            if file.endswith('.pyc') or file.endswith('.log') or file.endswith('.sqlite') or file.endswith('.db') or file.endswith('.bak') or file.endswith('.pdf') or file.endswith('.docx') or file.endswith('.xlsx'):
                continue
...
def main():
    ...
    ftp.quit()
    logging.info("Remaining modules deployment complete.")
