...
try:
    ...
    print("Argon2 NOT found. Falling back to simple SHA256 (not recommended but functional for test).")
    import hashlib
    PH = None
...
def hash_pass(plain):
    if PH:
        ...
        # Fallback to simple SHA256 just to get in if Argon2 fails hard
...
def main():
    ...
    if not row:
        ...
        cursor.execute("INSERT INTO users (username, password_hash, display_name, email, is_active) VALUES (?, ?, ?, ?, 1)",
