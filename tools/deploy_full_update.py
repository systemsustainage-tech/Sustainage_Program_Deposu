...
def deploy():
    try:
        ...
        # 4. Deploy and Run Populate Full Test Data
        local_pop = r'C:\SUSTAINAGESERVER\tools\populate_full_test_data.py'
        remote_pop = '/var/www/sustainage/populate_full_test_data.py'
        print(f"Uploading {local_pop}...")
        sftp.put(local_pop, remote_pop)
        
        print("Running Populate Full Test Data...")
        stdin, stdout, stderr = client.exec_command(f'python3 {remote_pop}')
        print(stdout.read().decode())
        print(stderr.read().decode())

        # 5. Install Reportlab (Directly)
        print("Installing Reportlab...")
        # Try apt first
        stdin, stdout, stderr = client.exec_command('apt-get update && apt-get install -y python3-reportlab')
        print("Apt output:", stdout.read().decode())
        print("Apt error:", stderr.read().decode())
        
        # Try pip fallback
        stdin, stdout, stderr = client.exec_command('pip3 install reportlab --break-system-packages')
        print("Pip output:", stdout.read().decode())
        print("Pip error:", stderr.read().decode())
