...
def run_command(command, is_ssh=False):
    ...
    if is_ssh:
        ...
        if len(parts) == 2:
            full_cmd = f"{parts[0]}'{PATH_EXPORT} {parts[1]}"
        else:
            # Maybe double quotes?
            parts = command.split('"', 1)
            if len(parts) == 2:
                ...
                print("Warning: Could not inject PATH, running as is.")
...
def fix_database_final():
    print("--- Fixing Remote Database Final ---")
    ...
    # scp doesn't take commands, so no PATH injection needed
