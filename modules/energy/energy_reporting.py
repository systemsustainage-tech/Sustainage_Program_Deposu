...
class EnergyReporting:
    ...
    def generate_energy_report(self, company_id: int, period: str,
        ...
        records = []
        if hasattr(self.energy_manager, 'get_energy_records'):
            records = self.energy_manager.get_energy_records(company_id, year)
        
        # Metrics hesaplama
        metrics = {}
        if hasattr(self.energy_manager, 'calculate_energy_kpis'):
            metrics = self.energy_manager.calculate_energy_kpis(company_id, year)
    ...
    def _generate_docx_report(self, company_id: int, period: str, records: List[Dict], 
        ...
        try:
            ...
            if metrics:
                ...
                metrics_data = [
                    ('Toplam Tüketim', f"{metrics.get('total_energy_consumption', 0):,.2f} kWh"),
                    ('Toplam Maliyet', f"{metrics.get('energy_cost', 0):,.2f} TL"),
                    ('Yenilenebilir Oranı', f"%{metrics.get('renewable_energy_ratio', 0):.1f}"),
                    ('Enerji Yoğunluğu (Çalışan)', f"{metrics.get('energy_intensity_per_employee', 0):.2f}")
                ]
            ...
            if include_details and records:
                ...
                for record in records:
                    ...
                    row_cells[0].text = str(record.get('created_at', '')[:10]) # Use created_at if measurement_date not available
        ...
        try:
            ...
            if records:
                ...
                headers = [
                    ...
                    'Maliyet', 'Fatura Tarihi', 'Son Ödeme Tarihi', 'Tedarikçi'
                ]
                ...
                for row, record in enumerate(records, 2):
                    ws_records.cell(row=row, column=1, value=str(record.get('created_at', '')[:10]))
                    ws_records.cell(row=row, column=2, value=record.get('energy_type', ''))
                    ws_records.cell(row=row, column=3, value=record.get('source', ''))
                    ...
                    ws_records.cell(row=row, column=7, value=record.get('invoice_date', ''))
                    ws_records.cell(row=row, column=8, value=record.get('due_date', ''))
                    ws_records.cell(row=row, column=9, value=record.get('supplier', ''))
