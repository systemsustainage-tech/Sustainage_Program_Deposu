...
def license_check():
    ...
    def check_restrictions(payload):
        """Helper to validate IP and Domain restrictions from license payload."""
        # Domain Check
    ...
    if api_key:
        ...
        # Check Restrictions
        is_allowed, denial_msg = check_restrictions(payload)
        if not is_allowed:
            return jsonify({'error': denial_msg}), 403
...
def super_admin_audit_logs():
    ...
    try:
        ...
        ts_col_a = 'timestamp' if 'timestamp' in cols_a else ('created_at' if 'created_at' in cols_a else 'ts')
        det_col_a = 'details' if 'details' in cols_a else ('payload_json' if 'payload_json' in cols_a else 'metadata')
        
        # Fallback if columns missing
        if ts_col_a not in cols_a: ts_col_a = "datetime('now')"
        if det_col_a not in cols_a: det_col_a = "''"
        ...
        query_a = f"""
            SELECT 
                ...
                a.action,
                COALESCE(a.{ts_col_a}, datetime('now')) as ts,
                COALESCE(a.{det_col_a}, '') as details
            FROM audit_logs a
        ...
        if cols_s:
            ...
            if 'user_id' in cols_s:
                 where_s = "WHERE user_id IN (SELECT id FROM users WHERE company_id = ?)"
                 params_s.append(g.company_id)
            elif 'company_id' in cols_s:
                 where_s = "WHERE company_id = ?"
                 params_s.append(g.company_id)
            else:
                 # If no company link, maybe don't show or show all? 
                 # Showing all might leak data in SaaS. 
                 # Let's assume we skip if no company context can be established
                 where_s = "WHERE 0=1" 
...
def training_module():
    ...
    records = manager.get_training_records(company_id)
    
    return render_template('training.html', title='Eğitim Yönetimi', manager_available=True, programs=programs, records=records)
...
def human_rights_module():
    ...
    # Placeholder stats until specific HumanRightsManager is implemented
    stats = {'incidents': 0, 'audits': 0, 'violations': 0}
    
    if manager and hasattr(manager, 'get_human_rights_stats'):
        try:
            stats = manager.get_human_rights_stats(company_id)
        except Exception as e:
            logging.error(f"Error in human rights stats: {e}")
            
    return render_template('human_rights.html', title=_('human_rights_title', 'İnsan Hakları'), stats=stats, manager_available=bool(manager))
...
def labor_module():
    ...
    stats = {'workforce': 0, 'turnover': 0, 'injuries': 0}
    
    if manager and hasattr(manager, 'get_labor_stats'):
        try:
            stats = manager.get_labor_stats(company_id)
        except Exception as e:
            logging.error(f"Error in labor stats: {e}")
            
    return render_template('labor.html', title=_('labor_practices_title', 'Çalışma Uygulamaları'), stats=stats, manager_available=bool(manager))
...
def consumer_module():
    ...
    stats = {'complaints': 0, 'recalls': 0, 'satisfaction': 0}
    
    if manager and hasattr(manager, 'get_consumer_stats'):
        try:
            stats = manager.get_consumer_stats(company_id)
        except Exception as e:
            logging.error(f"Error in consumer stats: {e}")
            
    return render_template('consumer.html', title=_('consumer_issues_title', 'Tüketici Sorunları'), stats=stats, manager_available=bool(manager))
...
def community_module():
    ...
    stats = {'investments': 0, 'beneficiaries': 0, 'volunteering_hours': 0}
    
    if manager and hasattr(manager, 'get_community_stats'):
        try:
            stats = manager.get_community_stats(company_id)
        except Exception as e:
            logging.error(f"Error in community stats: {e}")
            
    return render_template('community.html', title=_('community_involvement_title', 'Toplumsal Katılım'), stats=stats, manager_available=bool(manager))
...
def supply_chain_profile(supplier_id):
    ...
    return render_template('supply_chain_profile.html', supplier=supplier, assessments=assessments, recommendations=recommendations, today_date=datetime.now().strftime('%Y-%m-%d'))
