...
class WasteManager:
    ...
    def __init__(self, db_path: str = None) -> None:
        ...
        # Calculator'ı başlat
        self.calculator = WasteCalculator()

        # Tabloları oluştur
        self.create_waste_tables()
        # Verileri doldur
        self.populate_waste_types()
    ...
    def add_waste_record(self, company_id: int, form_data: dict) -> bool:
        ...
        try:
            ...
            recycling_rate = float(form_data.get('recycling_rate', 0.0))
            disposal_method = form_data.get('disposal_method', '')
            disposal_cost = float(form_data.get('disposal_cost', 0.0))
            carbon_footprint = float(form_data.get('carbon_footprint', 0.0))
            record_date = form_data.get('record_date', datetime.now().strftime('%Y-%m-%d'))
            ...
            period = record_date[:4] if record_date else str(datetime.now().year)
            ...
            cursor.execute("""
                ...
                (company_id, period, waste_type_id, waste_code, waste_name, quantity, unit,
                 ...
                 invoice_date, due_date, supplier, notes)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (company_id, period, waste_type_id, waste_code, waste_name, quantity, unit,
                  disposal_method, recycling_rate, disposal_cost, carbon_footprint,
                  invoice_date, due_date, supplier, notes))
