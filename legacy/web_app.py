...
from backend.security.alert_system import report_violation, email_service
...
from backend.modules.super_admin.maintenance_checks import MaintenanceManager
...
SYSTEM_CPU_USAGE = Gauge('system_cpu_usage', 'Current system CPU usage percent')
SYSTEM_MEMORY_USAGE = Gauge('system_memory_usage', 'Current system memory usage percent')
...
def check_license_restrictions():
    # Only check if X-License-Key header is present
    license_key = request.headers.get('X-License-Key')
    if not license_key:
        return # Continue normal flow
        
    # Verify license
    is_valid, message, payload = license_manager.verify_license_key(license_key)
    if not is_valid:
        # If explicitly providing a key and it's invalid, we should probably reject
        return jsonify({'error': f'License Error: {message}'}), 403
        
    # Check IP restrictions
    allowed_ips = payload.get('allowed_ips')
    if allowed_ips:
        client_ip = get_remote_address()
        # Handle proxy headers if configured, but get_remote_address usually does.
    ...
    if allowed_domains:
        # Check Origin or Host
        # origin = request.headers.get('Origin')
        # We strip protocol and port for comparison
        origin = request.headers.get('Origin', '')
        if '://' in origin:
        ...
        if origin and origin in allowed_domains:
            domain_match = True
        # If no Origin (e.g. server-to-server), check if the Host itself is allowed (self-call)
        # OR if the request came from an allowed domain (but we can't know for sure without Origin/Referer)
        # We'll rely on Host as a fallback if Origin is missing, assuming the license binds the SERVER domain.
        elif not origin and request_domain in allowed_domains:
            domain_match = True
            
        if not domain_match:
             return jsonify({'error': 'Access denied: Domain not allowed'}), 403

    # Abuse detection
    is_abusive, reason = license_manager.update_usage_and_check_abuse(license_key)
    if is_abusive:
        license_manager.suspend_license(license_key, reason)
        # Send email
        try:
             # Force email to kivanc.kasoglu@sustainage.tr
             email_service.send_email(
                 to_email="kivanc.kasoglu@sustainage.tr",
                 subject="URGENT: License Suspended due to Abuse",
                 body=f"""
                 <h2>License Suspended</h2>
                 <p><strong>License Key:</strong> {license_key}</p>
                 <p><strong>Reason:</strong> {reason}</p>
                 <p><strong>Company ID:</strong> {payload.get('company_id')}</p>
                 <p><strong>IP:</strong> {get_remote_address()}</p>
                 <p><strong>Time:</strong> {datetime.now()}</p>
                 """
             )
        except Exception as e:
            logging.error(f"Could not send suspension email: {e}")
...
# Get limits from environment or use default
# default_limit_str = os.environ.get('RATE_LIMIT_DEFAULT', "100000 per day, 20000 per minute")
# UPDATED: Stricter default limits per user request
default_limit_str = "10000 per day, 200 per minute"
default_limits = [limit.strip() for limit in default_limit_str.split(',')]
...
def ratelimit_handler(e):
    ...
    try:
        report_violation(
            ...
            source_ip=client_ip, 
            details={"url": request.url, "error": str(e.description), "severity": "medium"}
        )
    except Exception as log_err:
        app.logger.error(f"Failed to log rate limit violation: {log_err}")
        
    # Return JSON for API, or Template for HTML? 
    # Since this is a mixed app, check request path or accept header.
    if request.path.startswith('/api/') or request.is_json:
        return jsonify(error="Too Many Requests", message=str(e.description)), 429
    return render_template('error_429.html', error=e.description), 429

# reCAPTCHA Configuration
# NOTE: These are dummy test keys for development. Replace with real keys in production.
app.config['RECAPTCHA_SITE_KEY'] = os.environ.get('RECAPTCHA_SITE_KEY', '6LeIxAcTAAAAAJcZVRqyHh71UMIEGNQ_MXjiZKhI') # Google Test Key
app.config['RECAPTCHA_SECRET_KEY'] = os.environ.get('RECAPTCHA_SECRET_KEY', '6LeIxAcTAAAAAGG-vFI1TnRWxMZNFuojJ4WifJWe') # Google Test Secret
...
def license_check():
    ...
    def check_constraints(payload):
        ...
        if allowed_ips and isinstance(allowed_ips, list) and len(allowed_ips) > 0:
            # Handle X-Forwarded-For explicitly for Nginx/Proxy support
...
def api_translations():
    ...
    # ETag and Cache-Control for efficient caching
    ...
    # Force revalidation with server using ETag
...
def api_dashboard_stats():
    ...
    return jsonify({
        'alerts': 0, 
        'next_deadline': '2025-12-31', # Placeholder
        'modules': module_data
    })
    ...
    if request.method == 'POST':
        ...
        if captcha_required:
            ...
            if recaptcha_response:
                ...
                if not captcha_manager.verify_google_recaptcha(recaptcha_response, app.config.get('RECAPTCHA_SECRET_KEY'), client_ip):
                    flash('reCAPTCHA doğrulaması başarısız. Lütfen tekrar deneyin.', 'danger')
                    report_violation("CAPTCHA_FAIL", client_ip, {"username": username, "type": "google"})
                    return render_template('login.html', captcha_required=True, captcha_code=session.get('captcha_code'), recaptcha_site_key=app.config.get('RECAPTCHA_SITE_KEY'))
            else:
                # Fallback to Custom CAPTCHA
                ...
                if not captcha_manager.verify_captcha(session.get('captcha_code'), user_captcha):
                    ...
                    report_violation("CAPTCHA_FAIL", client_ip, {"username": username, "type": "custom"})
                    return render_template('login.html', captcha_required=True, captcha_code=session.get('captcha_code'), recaptcha_site_key=app.config.get('RECAPTCHA_SITE_KEY')) # Reload new captcha handled by template calling endpoint
        ...
        if not is_privileged:
            ...
            if not rl.get('allowed', True):
                ...
                return render_template('login.html', captcha_required=True, recaptcha_site_key=app.config.get('RECAPTCHA_SITE_KEY')), 429
        ...
        try:
            ...
            if not is_privileged:
                ...
                if not can_login:
                    ...
                    return render_template('login.html', captcha_required=captcha_required, recaptcha_site_key=app.config.get('RECAPTCHA_SITE_KEY'))
        except Exception as e:
            logging.error(f"Lock check error: {e}")
...
def forgot_password():
    ...
    # Always require CAPTCHA for password reset to prevent email spam/DoS
    captcha_required = True
    
    if request.method == 'POST':
        client_ip = request.remote_addr
        
        # 1. reCAPTCHA Verification (if configured)
        if recaptcha_site_key:
            recaptcha_response = request.form.get('g-recaptcha-response')
            if not recaptcha_response:
                flash('Lütfen reCAPTCHA doğrulamasını tamamlayın.', 'warning')
                return render_template('forgot_password.html', captcha_required=True, recaptcha_site_key=recaptcha_site_key)
            
            if not captcha_manager.verify_google_recaptcha(recaptcha_response, app.config.get('RECAPTCHA_SECRET_KEY'), client_ip):
                flash('reCAPTCHA doğrulaması başarısız. Lütfen tekrar deneyin.', 'danger')
                report_violation("CAPTCHA_FAIL", client_ip, {"action": "forgot_password", "type": "google"})
                return render_template('forgot_password.html', captcha_required=True, recaptcha_site_key=recaptcha_site_key)
        
        # 2. Custom CAPTCHA Verification (fallback)
        else:
            user_captcha = request.form.get('captcha', '')
            if not captcha_manager.verify_captcha(session.get('captcha_code'), user_captcha):
                flash('Güvenlik kodu hatalı. Lütfen tekrar deneyin.', 'danger')
                report_violation("CAPTCHA_FAIL", client_ip, {"action": "forgot_password", "type": "custom"})
    ...
    return render_template('forgot_password.html', captcha_required=captcha_required, recaptcha_site_key=recaptcha_site_key)
...
@app.route('/super_admin/maintenance', methods=['GET', 'POST'])
@super_admin_required
def super_admin_maintenance():
    ...
    # Handle Check Execution
    if request.method == 'POST':
        try:
            checker = MaintenanceManager(DB_PATH)
            results = checker.run_all_checks()
            flash('Sistem kontrolleri tamamlandı.', 'success')
        except Exception as e:
            logging.error(f"Maintenance check error: {e}")
            flash(f'Kontrol sırasında hata: {e}', 'danger')
...
def social_add():
    ...
    if request.method == 'POST':
        try:
            ...
            if dtype == 'employee':
            ...
            elif dtype == 'ohs' or dtype == 'incident':
                ...
                hours = float(request.form.get('hours') or request.form.get('total_hours', 0))
