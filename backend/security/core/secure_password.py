...
try:
    ...
    HAS_ARGON2 = False
    logging.warning("argon2 module not found, falling back to pbkdf2")
...
def hash_password(password: str) -> str:
    ...
    if HAS_ARGON2:
        try:
            ph = PasswordHasher()
            h = ph.hash(password)
            # argon2-cffi returns string directly
            if h.startswith("$"):
                return "argon2$" + h
            return "argon2$" + h 
        except Exception as e:
            logging.error(f"Argon2 hashing failed: {e}")
    ...
    try:
        ...
        if stored_hash.startswith("argon2_sha256$"):
            ...
            if HAS_ARGON2:
                try:
                    ph = PasswordHasher()
                    ph.verify(inner_hash, hashed_input)
                    return True, False 
        ...
        if s.startswith("$argon2"):
            if HAS_ARGON2:
                try:
                    ...
                    # If verified, check if we need rehash (e.g. parameters changed) - ignoring for now
                    return True, False
                except VerifyMismatchError:
                    return False, False
                except Exception as e:
                    logging.error(f"Argon2 verify error: {e}")
                    return False, False
        ...
        if stored_hash.startswith("pbkdf2$"):
            ...
            if ":" in payload:
                ...
                return calc == h, True # Needs rehash to upgrade to Argon2
            
        # 4. Legacy SHA-256 (Hex)
        if len(stored_hash) == 64 and all(c in "0123456789abcdefABCDEF" for c in stored_hash):
            return hashlib.sha256(password.encode("utf-8")).hexdigest() == stored_hash, True
...
def _ensure_security_logs(conn) -> None:
    cur = conn.cursor()
    cur.execute(
        """
...
def _insert_security_log(cur, action: str, username: str, success: bool, ip_address: str, metadata: dict, user_id: int = None) -> None:
    try:
        cur.execute(
...
def _get_system_setting_int(key: str, default: int) -> int:
    try:
        # Use absolute path to ensure DatabaseManager finds it correctly
        db_path = os.path.abspath("backend/data/sdg_desktop.sqlite")
        db = DatabaseManager(db_path)
        with db.get_connection() as conn:
            cur = conn.cursor()
            cur.execute(
...
def check_password_history(db_path: str, username: str, new_password: str, max_history: int = 5) -> bool:
    ...
    # We need to manually verify against each hash
    # This might be slow if argon2 is used, but history check is rare
    for h in rows:
         ok, _ = verify_password(h, new_password)
         if ok:
             return False # Found in history, so invalid
