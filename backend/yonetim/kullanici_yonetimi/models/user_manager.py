...
# from services.email_service import EmailService  # Eski/Yanlış import
try:
...
class UserManager:
    ...
    def __init__(self, db_path: str = DB_PATH) -> None:
        ...
        # DEBUG LOGGING
        # print(f"DEBUG: UserManager initialized with db_path: {self.db_path}", flush=True)
    ...
    def get_user_company_ids(self, user_id: int) -> List[int]:
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("SELECT company_id FROM user_companies WHERE user_id = ?", (user_id,))
    ...
    def authenticate(self, username, password) -> Optional[Dict]:
        ...
        try:
            ...
            logging.error(f"Auth attempt for {username}. Hash prefix: {stored_hash[:30] if stored_hash else 'None'}")
    ...
    def create_user(self, user_data: Dict, created_by: Optional[int] = None) -> int:
        ...
        try:
            ...
            # Şirketleri ata (varsa)
            if 'company_ids' in user_data and user_data['company_ids']:
                for idx, company_id in enumerate(user_data['company_ids']):
                    is_primary = 1 if idx == 0 else 0  # İlk seçilen primary olsun
                    cursor.execute(
                        """
                        INSERT INTO user_companies (user_id, company_id, is_primary, assigned_by)
                        VALUES (?, ?, ?, ?)
                        """,
                        (user_id, company_id, is_primary, created_by),
                    )
            elif 'company_id' in user_data: # Tekli seçim desteği (legacy)
                 cursor.execute(
                    """
                    INSERT INTO user_companies (user_id, company_id, is_primary, assigned_by)
                    VALUES (?, ?, 1, ?)
                    """,
                    (user_id, user_data['company_id'], created_by),
                )
            else:
                # Varsayılan şirketi ata (ID: 1)
                try:
                     cursor.execute(
                        """
                        INSERT OR IGNORE INTO user_companies (user_id, company_id, is_primary, assigned_by)
            ...
            try:
                ...
                self.email_service.send_new_user_credentials(
                ...
                # logging.error(f'Silent error in user_manager.py email sending: {str(e)}')
                # Re-raise or log strictly
                logging.error(f"CRITICAL EMAIL ERROR in create_user: {e}", exc_info=True)
                # Opsiyonel: Hata fırlatılabilir ama kullanıcı oluşturmayı bozmamak için logluyoruz
    ...
    def update_user(self, user_id: int, user_data: Dict, updated_by: Optional[int] = None) -> bool:
        ...
        try:
            ...
            # Şirketleri güncelle
            if 'company_ids' in user_data:
                # Mevcut şirketleri sil
                ...
                for idx, company_id in enumerate(user_data['company_ids']):
                    ...
                    cursor.execute("""
                        INSERT INTO user_companies (user_id, company_id, is_primary, assigned_by)
                        VALUES (?, ?, ?, ?)
                    """, (user_id, company_id, is_primary, updated_by))
        ...
        try:
            ...
            if user_row:
                ...
                if username == '__super__':
                    logging.warning(f"Attempt to delete immutable super user {username} blocked.")
                    return False
                try:
    ...
    def _log_audit(self, cursor, user_id: Optional[int], action: str, resource_type: str,
        ...
        try:
            ...
            # payload_json removed as it is not in the schema
