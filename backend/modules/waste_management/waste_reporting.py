...
class WasteReporting:
        ...
        try:
            ...
            try:
                ...
                if include_details and waste_records:
                    _add_turkish_heading(doc, 'Atık Kayıtları', level=1)
                    records_table = doc.add_table(rows=1, cols=9)
                    ...
                    hdr_cells[6].text = 'Fatura Tarihi'
                    hdr_cells[7].text = 'Son Ödeme'
                    hdr_cells[8].text = 'Tedarikçi'
                    ...
                    for record in waste_records:
                        ...
                        row_cells[6].text = record.get('invoice_date', '') or '-'
                        row_cells[7].text = record.get('due_date', '') or '-'
                        row_cells[8].text = record.get('supplier', '') or '-'

                # Hedefler
                if waste_targets:
                    _add_turkish_heading(doc, 'Atık Azaltma Hedefleri', level=1)
                ...
                if recycling_projects:
                    _add_turkish_heading(doc, 'Geri Dönüşüm Projeleri', level=1)
                ...
                # İstatistikler
                if statistics:
                    _add_turkish_heading(doc, 'Genel İstatistikler', level=1)
                    stats_para = _add_turkish_paragraph(doc, )
                ...
                doc.save(filepath)
                logging.info(f"[OK] Atik yonetimi raporu olusturuldu: {filepath}")
                ...
                logging.info("[UYARI] python-docx yuklu degil. DOCX rapor olusturulamadi.")
                return None

        except Exception as e:
            logging.error(f"[HATA] Atik yonetimi raporu olusturulurken hata: {e}")
            return None
        ...
        try:
            ...
            if waste_records:
                ...
                headers = ['Atık Kodu', 'Atık Adı', 'Miktar', 'Birim', 'Kategori',
                          ...
                          'Fatura Tarihi', 'Son Ödeme Tarihi', 'Tedarikçi']
                ...
                for row, record in enumerate(waste_records, 2):
                    ...
                    ws_records.cell(row=row, column=15, value=record.get('invoice_date', ''))
                    ws_records.cell(row=row, column=16, value=record.get('due_date', ''))
                    ws_records.cell(row=row, column=17, value=record.get('supplier', ''))
            ...
            wb.save(filepath)
            logging.info(f"[OK] Excel atik yonetimi raporu olusturuldu: {filepath}")
            ...
            logging.info("[UYARI] openpyxl yuklu degil. Excel rapor olusturulamadi.")
            return None
        except Exception as e:
            logging.error(f"[HATA] Excel atik yonetimi raporu olusturulurken hata: {e}")
            return None
    ...
    def generate_csv_report(self, company_id: int, period: str) -> Optional[str]:
        ...
        try:
            ...
            if not waste_records:
                logging.info("[UYARI] Atik kaydi bulunamadi.")
                return None
            ...
            logging.info(f"[OK] CSV atik yonetimi raporu olusturuldu: {filepath}")
            ...
            logging.error(f"[HATA] CSV atik yonetimi raporu olusturulurken hata: {e}")
            return None
    ...
    def save_waste_report(self, company_id: int, report_name: str, report_type: str,
        ...
        try:
            ...
            conn.commit()
            logging.info(f"[OK] Atik raporu veritabanina kaydedildi: {report_name}")
            return report_id

        except Exception as e:
            logging.error(f"[HATA] Atik raporu kaydedilirken hata: {e}")
            return None
        finally:
            conn.close()
    ...
    def get_waste_reports(self, company_id: int) -> List[Dict]:
        ...
        try:
            ...
            logging.error(f"[HATA] Atik raporlari getirilirken hata: {e}")
            return []
        finally:
            conn.close()
