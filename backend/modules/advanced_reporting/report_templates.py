...
class AdvancedReportTemplates:
    ...
    def create_sdg_report(self, company_id: int, period: str, data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logging.info(f"[OK] SDG raporu oluşturuldu: {file_path}")
            return file_path
    ...
    def create_gri_report(self, company_id: int, period: str, data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # Başlık
            title = _add_turkish_heading(doc, 'GRI SÜRDÜRÜLEBİLİRLİK RAPORU', 0)
            title.alignment = WD_ALIGN_PARAGRAPH.CENTER

            # Şirket bilgileri
            _add_turkish_paragraph(doc, f"Şirket: {data.get('company_name', 'Bilinmeyen')}")
            _add_turkish_paragraph(doc, f"Dönem: {period}")
            _add_turkish_paragraph(doc, f"Rapor Tarihi: {datetime.now().strftime('%d.%m.%Y')}")
            _add_turkish_paragraph(doc, "")

            # GRI konuları
            if 'gri_topics' in data:
                _add_turkish_heading(doc, 'GRI Konuları', level=1)

                for topic, details in data['gri_topics'].items():
                    _add_turkish_heading(doc, topic, level=2)
                    _add_turkish_paragraph(doc, f"Açıklama: {details.get('description', '')}")
                    _add_turkish_paragraph(doc, f"Kapsam: {details.get('scope', '')}")
                    _add_turkish_paragraph(doc, f"Metodoloji: {details.get('methodology', '')}")
                    _add_turkish_paragraph(doc, "")
            ...
            logging.info(f"[OK] GRI raporu oluşturuldu: {file_path}")
            return file_path
    ...
    def create_carbon_report(self, company_id: int, period: str, data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logging.info(f"[OK] Karbon raporu oluşturuldu: {file_path}")
            return file_path
    ...
    def get_generation_history(self, company_id: int = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logging.error(f"[HATA] Rapor geçmişi alınamadı: {e}")
            return []
...
if __name__ == "__main__":
    ...
    logging.info(f"SDG raporu: {sdg_report}")
    ...
    logging.info(f"GRI raporu: {gri_report}")
    ...
    logging.info(f"Karbon raporu: {carbon_report}")
