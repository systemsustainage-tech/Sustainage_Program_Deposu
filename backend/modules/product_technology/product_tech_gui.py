...
class ProductTechGUI:
    ...
    def setup_ui(self) -> None:
        ...
        title_label = tk.Label(title_frame, text=self.lm.tr("product_tech_module_title", "Ürün ve Teknoloji Modülü"),
        ...
        # Modül başlığı
        module_title = tk.Label(left_frame, text=self.lm.tr("modules", "Modüller"), font=('Segoe UI', 12, 'bold'),
                               bg='white', fg='#e67e22')
        ...
        modules = [
            ("", self.lm.tr("module_innovation", "AR-GE ve İnovasyon"), self.show_innovation),
            (Icons.STAR, self.lm.tr("module_quality", "Kalite Yönetimi"), self.show_quality),
            ("", self.lm.tr("module_security", "Dijital Güvenlik"), self.show_security),
            ("", self.lm.tr("module_emergency", "Acil Durum Yönetimi"), self.show_emergency)
        ]
        ...
        # Detay başlığı
        self.detail_title = tk.Label(right_frame, text=self.lm.tr("details", "Detaylar"), font=('Segoe UI', 12, 'bold'),
                                    bg='white', fg='#e67e22')
    ...
    def show_summary(self) -> None:
        ...
        self.detail_title.config(text=self.lm.tr("module_summary", "Modül Özeti"))
        ...
        if not summary['has_data']:
            info_label = tk.Label(self.detail_frame,
                                 text=self.lm.tr("no_data_select_module", "Henüz veri girilmemiş. Sol menüden bir modül seçerek veri girebilirsiniz."),
        ...
        # Modül durumları
        modules = [
            (self.lm.tr("module_innovation", "AR-GE ve İnovasyon"), summary['innovation'], '#3498db'),
            (self.lm.tr("module_quality", "Kalite Yönetimi"), summary['quality'], '#2ecc71'),
            (self.lm.tr("module_security", "Dijital Güvenlik"), summary['security'], '#9b59b6'),
            (self.lm.tr("module_emergency", "Acil Durum Yönetimi"), summary['emergency'], '#e74c3c')
        ]
        ...
        for i, (name, data, color) in enumerate(modules):
            if data:
                ...
                status_label = tk.Label(card, text=self.lm.tr("data_available", "Veri Mevcut"), font=('Segoe UI', 10),
                                      fg='white', bg=color)
    ...
    def show_innovation(self) -> None:
        ...
        self.clear_detail()
        self.detail_title.config(text=self.lm.tr("module_innovation", "AR-GE ve İnovasyon"))
        ...
        # Başlık
        title_label = tk.Label(form_frame, text=self.lm.tr("innovation_metrics", "AR-GE ve İnovasyon Metrikleri"),
                              font=('Segoe UI', 14, 'bold'), bg='white', fg='#e67e22')
        title_label.pack(pady=(0, 20))

        # Form alanları
        fields = [
            (self.lm.tr("rd_investment_ratio", "Ar-Ge Yatırım Oranı (%)"), "rd_investment_ratio", "float"),
            (self.lm.tr("patent_applications", "Patent Başvuru Sayısı"), "patent_applications", "int"),
            (self.lm.tr("ecodesign_integration", "Eko-Tasarım Entegrasyonu"), "ecodesign_integration", "bool"),
            (self.lm.tr("lca_implementation", "LCA Uygulaması"), "lca_implementation", "bool"),
            (self.lm.tr("innovation_budget", "İnovasyon Bütçesi (TL)"), "innovation_budget", "float"),
            (self.lm.tr("reporting_period", "Raporlama Dönemi"), "reporting_period", "str")
        ]
    ...
    def save_innovation_metrics(self) -> None:
        ...
        try:
            ...
            if success:
                messagebox.showinfo(self.lm.tr("success", "Başarılı"), self.lm.tr("innovation_saved", "AR-GE metrikleri kaydedildi"))
            else:
                messagebox.showerror(self.lm.tr("error", "Hata"), self.lm.tr("innovation_save_error", "AR-GE metrikleri kaydedilemedi"))

        except ValueError as e:
            messagebox.showerror(self.lm.tr("error", "Hata"), f"{self.lm.tr('invalid_data', 'Geçersiz veri')}: {str(e)}")

    def show_quality(self) -> None:
        """Kalite Yönetimi modülünü göster"""
        self.clear_detail()
        self.detail_title.config(text=self.lm.tr("module_quality", "Kalite Yönetimi"))
        ...
        # Başlık
        title_label = tk.Label(form_frame, text=self.lm.tr("quality_metrics", "Kalite Yönetimi Metrikleri"),
                              font=('Segoe UI', 14, 'bold'), bg='white', fg='#e67e22')
        title_label.pack(pady=(0, 20))

        # Form alanları
        fields = [
            (self.lm.tr("iso9001_certified", "ISO 9001 Sertifikası"), "iso9001_certified", "bool"),
            (self.lm.tr("customer_complaint_rate", "Müşteri Şikayet Oranı (%)"), "customer_complaint_rate", "float"),
            (self.lm.tr("product_recall_count", "Ürün Geri Çağırma Sayısı"), "product_recall_count", "int"),
            (self.lm.tr("nps_score", "NPS Skoru"), "nps_score", "float"),
            (self.lm.tr("quality_error_rate", "Kalite Hata Oranı (%)"), "quality_error_rate", "float"),
            (self.lm.tr("reporting_period", "Raporlama Dönemi"), "reporting_period", "str")
        ]
    ...
    def save_quality_metrics(self) -> None:
        ...
        try:
            ...
            if success:
                messagebox.showinfo(self.lm.tr("success", "Başarılı"), self.lm.tr("quality_saved", "Kalite metrikleri kaydedildi"))
            else:
                messagebox.showerror(self.lm.tr("error", "Hata"), self.lm.tr("quality_save_error", "Kalite metrikleri kaydedilemedi"))
    ...
    def show_security(self) -> None:
        ...
        self.clear_detail()
        self.detail_title.config(text=self.lm.tr("module_security", "Dijital Güvenlik"))
        ...
        # Başlık
        title_label = tk.Label(form_frame, text=self.lm.tr("security_metrics", "Dijital Güvenlik Metrikleri"),
                              font=('Segoe UI', 14, 'bold'), bg='white', fg='#e67e22')
        title_label.pack(pady=(0, 20))

        # Form alanları
        fields = [
            (self.lm.tr("iso27001_certified", "ISO 27001 Sertifikası"), "iso27001_certified", "bool"),
            (self.lm.tr("cybersecurity_training_hours", "Siber Güvenlik Eğitim Saati"), "cybersecurity_training_hours", "int"),
            (self.lm.tr("data_breach_count", "Veri İhlali Sayısı"), "data_breach_count", "int"),
            (self.lm.tr("digital_transformation_score", "Dijital Dönüşüm Skoru"), "digital_transformation_score", "float"),
            (self.lm.tr("ai_applications_count", "AI Uygulama Sayısı"), "ai_applications_count", "int"),
            (self.lm.tr("reporting_period", "Raporlama Dönemi"), "reporting_period", "str")
        ]
    ...
    def save_security_metrics(self) -> None:
        ...
        try:
            ...
            if success:
                messagebox.showinfo(self.lm.tr("success", "Başarılı"), self.lm.tr("security_saved", "Dijital güvenlik metrikleri kaydedildi"))
            else:
                messagebox.showerror(self.lm.tr("error", "Hata"), self.lm.tr("security_save_error", "Dijital güvenlik metrikleri kaydedilemedi"))
    ...
    def show_emergency(self) -> None:
        ...
        self.clear_detail()
        self.detail_title.config(text=self.lm.tr("module_emergency", "Acil Durum Yönetimi"))
        ...
        # Başlık
        title_label = tk.Label(form_frame, text=self.lm.tr("emergency_metrics", "Acil Durum Yönetimi Metrikleri"),
                              font=('Segoe UI', 14, 'bold'), bg='white', fg='#e67e22')
        title_label.pack(pady=(0, 20))

        # Form alanları
        fields = [
            (self.lm.tr("business_continuity_plan", "İş Sürekliliği Planı"), "business_continuity_plan", "bool"),
            (self.lm.tr("emergency_drill_frequency", "Acil Durum Tatbikat Sıklığı"), "emergency_drill_frequency", "int"),
            (self.lm.tr("risk_assessment_score", "Risk Değerlendirme Skoru"), "risk_assessment_score", "float"),
            (self.lm.tr("crisis_management_team", "Kriz Yönetim Ekibi"), "crisis_management_team", "bool"),
            (self.lm.tr("insurance_coverage_score", "Sigorta Kapsam Skoru"), "insurance_coverage_score", "float"),
            (self.lm.tr("reporting_period", "Raporlama Dönemi"), "reporting_period", "str")
        ]
    ...
    def save_emergency_metrics(self) -> None:
        ...
        try:
            ...
            if success:
                messagebox.showinfo(self.lm.tr("success", "Başarılı"), self.lm.tr("emergency_saved", "Acil durum metrikleri kaydedildi"))
            else:
                messagebox.showerror(self.lm.tr("error", "Hata"), self.lm.tr("emergency_save_error", "Acil durum metrikleri kaydedilemedi"))
