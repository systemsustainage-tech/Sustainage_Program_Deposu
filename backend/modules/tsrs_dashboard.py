...
class TSRSDashboard(BaseTenantManager):
    ...
    def _ensure_tables(self) -> None:
        ...
        try:
            # TSRS standartları tablosu (Global)
            self.execute_update("""
            ...
            # TSRS göstergeleri tablosu (Global)
            self.execute_update("""
            ...
            # TSRS yanıtları tablosu (Tenant-Specific)
            self.execute_update("""
    ...
    def get_dashboard_summary(self, company_id: int, period: int = 2024) -> Dict[str, Any]:
        ...
        try:
            # Toplam standart sayısı (Global)
            std_result = self.execute_query("SELECT COUNT(*) as count FROM tsrs_standards")
            total_standards = std_result[0]['count'] if std_result else 0

            # Toplam gösterge sayısı (Global)
            ind_result = self.execute_query("SELECT COUNT(*) as count FROM tsrs_indicators")
            total_indicators = ind_result[0]['count'] if ind_result else 0

            # Yanıtlanan gösterge sayısı (Tenant-Specific)
            ans_result = self.execute_query("""
                SELECT COUNT(DISTINCT indicator_id) as count
                FROM tsrs_responses 
                WHERE company_id = ? AND reporting_period = ?
            """, (company_id, str(period)), company_id=company_id)
            answered_indicators = ans_result[0]['count'] if ans_result else 0

            # Yanıt yüzdesi
            response_percentage = (answered_indicators / total_indicators * 100) if total_indicators > 0 else 0

            # Kategori bazında dağılım (Global)
            cat_result = self.execute_query("""
                SELECT category, COUNT(*) as count
                FROM tsrs_standards 
                GROUP BY category
            """)
            category_distribution = {row['category']: row['count'] for row in cat_result}

            # Son yanıtlar (Tenant-Specific)
            recent_responses = self.execute_query("""
            ...
            # Convert Row objects to dict/tuples for serialization if needed, 
            # or just return as list of dicts which is standard in this codebase refactoring
            recent_responses_list = [dict(row) for row in recent_responses]
    ...
    def get_category_standards(self, category: str) -> List[Dict]:
        """Kategoriye göre standartları getir (Global)"""
        try:
            standards = self.execute_query("""
            ...
            return [dict(row) for row in standards]
    ...
    def get_indicators_by_standard(self, standard_id: int) -> List[Dict]:
        """Standarta göre göstergeleri getir (Global)"""
        try:
            ...
            return [dict(row) for row in indicators]
    ...
    def save_indicator_response(self, company_id: int, indicator_id: int,
        ...
        """Gösterge yanıtını kaydet (Tenant-Specific)"""
        try:
            # Mevcut yanıtı kontrol et
            existing = self.execute_query("""
                SELECT id FROM tsrs_responses 
                WHERE company_id = ? AND indicator_id = ? AND reporting_period = ?
            """, (company_id, indicator_id, reporting_period), company_id=company_id)

            if existing:
                # Güncelle
                # BaseTenantManager will inject "AND company_id = ?" if we pass company_id, 
                # effectively making it "WHERE id = ? AND company_id = ?"
                self.execute_update("""
